...
def escalonar_prioridade_round_robin(processos, quantum, nome_arquivo="prioridade_round_robin"):
    ...
    print("Round Robin com Prioridade - Escalonamento")
    ...
    while processos_restantes or fila:
        ...
        if fila:
            maior_prioridade = min(p['priority'] for p in fila)
            prontos = [p for p in fila if p['priority'] == maior_prioridade]
            ...
            tempo += 1  # Avança o tempo se nada está pronto

    desenhar_gantt(gantt, nome_arquivo)

def desenhar_gantt(gantt, nome_arquivo="prioridade_round_robin"):
    os.makedirs("graficos", exist_ok=True)

    fig, gnt = plt.subplots()
    gnt.set_title("Gráfico de Gantt - Round Robin com Prioridade")
